 ...
 * 审查单个文件节点
 */
export async function reviewFileNode(
    state: ReviewState,
): Promise<Partial<ReviewState>> {
    const index = state.currentFileIndex;
    console.log(
        `🔍 [ReviewFileNode] Starting with currentFileIndex=${index}, relevantDiffs.length=${state.relevantDiffs.length}`,
    );
    console.log(
        `🔍 [ReviewFileNode] Full state:`,
        JSON.stringify({
    ...
    if (index >= state.relevantDiffs.length) {
        console.log(
            `⏭️ [ReviewFileNode] No more files to review (index=${index} >= length=${state.relevantDiffs.length})`,
        );
        // 返回一个标志，表示已经完成
        return {
            completed: true,
    ...
    console.log(
        `📄 [ReviewFileNode] Reviewing file [${index + 1}/${state.relevantDiffs.length}]: ${filePath}`,
    );
    ...
    const criticalComments = reviewComments.filter(
    ...
    console.log(
        `✅ [ReviewFileNode] Completed review for ${filePath}, NOT returning currentFileIndex`,
    );
    return result;
}
 ...
 * 检查是否还有更多文件需要审查
 */
export function shouldContinueReview(
    state: ReviewState,
): "continue" | "aggregate" {
    // 当前文件索引 + 1 表示下一个要审查的文件
    const nextIndex = state.currentFileIndex + 1;
    console.log(
        `🔍 [shouldContinueReview] currentFileIndex: ${state.currentFileIndex}, nextIndex: ${nextIndex}, totalDiffs: ${state.relevantDiffs.length}`,
    );
    console.log(
        `🔍 [shouldContinueReview] State snapshot:`,
        JSON.stringify({
            currentFileIndex: state.currentFileIndex,
            relevantDiffsLength: state.relevantDiffs.length,
            fileResultsLength: state.fileResults?.length || 0,
    ...
    if (state.currentFileIndex >= state.relevantDiffs.length) {
        console.log(
            `🏁 [shouldContinueReview] Current index already exceeds total, aggregating results`,
        );
        return "aggregate";
    }
    ...
    console.log(
        `🏁 [shouldContinueReview] All files reviewed, aggregating results`,
    );
    return "aggregate";
}
 ...
 * 移动到下一个文件
 */
export async function moveToNextFile(
    state: ReviewState,
): Promise<Partial<ReviewState>> {
    ...
    console.log(
        `➡️ [moveToNextFile] relevantDiffs.length=${state.relevantDiffs.length}`,
    );
    const result = {
