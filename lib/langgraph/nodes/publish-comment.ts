 ...
 * 发布评论节点
 */
export async function publishCommentNode(
    state: ReviewState,
    ...
    try {
        ...
        if (isPushEvent) {
            const pushMarker =
                ...
                `CRC_PUSH_PLACEHOLDER:${state.reviewLogId}`;
            let resolvedNoteId = reviewLog.gitlabNoteId || null;

            if (!resolvedNoteId) {
                try {
                    const commitComments =
                        await gitlabService.getCommitComments(
                            projectId,
                            reviewLog.commitSha,
                        );
                    const markerComment = [...commitComments]
                        ...
                        .find(
                            (item) =>
                                ...
                                item.note.includes(pushMarker),
                        );
                    const markerNoteId =
                        markerComment?.note_id || markerComment?.id || null;
                    if (markerNoteId) {
                        ...
                        console.log(
                            `📝 [PublishCommentNode] Resolved placeholder commit noteId=${resolvedNoteId} by marker=${pushMarker}`,
                        );
                    }
                } catch (error) {
                    console.warn(
                        `⚠️ [PublishCommentNode] Failed to resolve commit placeholder by marker`,
                        error,
                    );
                }
            }
            ...
            if (resolvedNoteId) {
                console.log(
                    `📝 [PublishCommentNode] Updating placeholder commit comment: noteId=${resolvedNoteId || reviewLog.gitlabNoteId}`,
                );
                try {
                    ...
                    console.warn(
                        `⚠️ [PublishCommentNode] Failed to update commit placeholder(noteId=${resolvedNoteId}), fallback to create summary`,
                        updateError,
                    );
                ...
                console.warn(
                    `⚠️ [PublishCommentNode] Unable to resolve placeholder by marker=${pushMarker}, fallback to create summary`,
                );
                result = (await gitlabService.createCommitComment(
                    projectId,
                    reviewLog.commitSha,
            ...
            if (resolvedDiscussionId && !resolvedNoteId) {
                try {
                    const discussion =
                        await gitlabService.getMergeRequestDiscussion(
                            projectId,
                    ...
                    if (
                        typeof firstNoteId === "number" &&
                        Number.isInteger(firstNoteId)
                    ) {
                        ...
                        console.log(
                            `📝 [PublishCommentNode] Resolved placeholder noteId=${resolvedNoteId} from discussion`,
                        );
                    }
                } catch (error) {
                    console.warn(
                        `⚠️ [PublishCommentNode] Failed to resolve placeholder noteId, fallback to create new summary`,
                        error,
                    );
                }
            }
            ...
            if (resolvedDiscussionId && resolvedNoteId) {
                console.log(
                    `📝 [PublishCommentNode] Updating placeholder MR comment: discussionId=${resolvedDiscussionId}`,
                );
...
function formatSummaryComment(
    ...
    const lines: string[] = [];
    const sortedComments = [...postedComments].sort(
        (a, b) => severityWeight(b.severity) - severityWeight(a.severity),
    );
    const criticalComments = sortedComments.filter(
        (item) => item.severity === "critical",
    );
    const normalComments = sortedComments.filter(
        (item) => item.severity === "normal",
    );
    const suggestionComments = sortedComments.filter(
    ...
    const filesWithIssues = fileResults.filter(
        (file) =>
            file.counts.critical > 0 ||
            file.counts.normal > 0 ||
            file.counts.suggestion > 0,
    ).length;
    ...
    if (reviewScope === "incremental") {
        lines.push(
            `- 审查模式：增量审查（基线 ${shortSha(incrementalBaseSha)} -> 当前 ${shortSha(reviewLog.commitSha)}）`,
        );
    } else {
        lines.push(`- 审查模式：全量审查（当前 MR/Commit 全部变更）`);
    }
    lines.push(
        `- 审查文件：${reviewedFiles}/${totalFiles}（其中 ${filesWithIssues} 个文件存在问题）`,
    );
    lines.push(
        `- 问题统计：🔴 严重 ${critical} / ⚠️ 一般 ${normal} / 💡 建议 ${suggestion}`,
    );
    ...
    if (actionableCount === 0) {
        ...
        [...criticalComments, ...normalComments].forEach((comment) => {
            ...
            lines.push(
                `- \`${location}\` (${comment.severity === "critical" ? "严重" : "一般"})`,
            );
        });
    }
    lines.push("");
    ...
    if (topFiles.length === 0) {
        ...
        for (const file of topFiles) {
            lines.push(
                `- \`${file.filePath}\`：🔴 ${file.counts.critical} / ⚠️ ${file.counts.normal} / 💡 ${file.counts.suggestion}`,
            );
        }
    }
    ...
    lines.push("");
    lines.push(
        `<sub>完成时间：${new Date().toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })}</sub>`,
    );
...
function getReviewConclusion(
    ...
    if (critical > 0)
        return `高风险：发现 ${critical} 个严重问题，建议修复后再合并`;
    if (normal > 0)
        return `中风险：无严重问题，但有 ${normal} 个一般问题需要关注`;
    if (suggestion > 0) return `低风险：仅有 ${suggestion} 条优化建议`;
    return "通过：未发现明显问题";
}

function parseStructuredFinding(content: string): {
    issue: string;
    impact: string;
    suggestion: string;
} {
    ...
    for (const segment of segments) {
        if (segment.startsWith("问题："))
            issue = segment.replace(/^问题：/, "").trim();
        if (segment.startsWith("影响："))
            impact = segment.replace(/^影响：/, "").trim();
        if (segment.startsWith("建议："))
            suggestion = segment.replace(/^建议：/, "").trim();
    }
