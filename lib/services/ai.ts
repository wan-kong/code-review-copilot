...
export class AIService {
    ...
    async reviewCode(
        ...
        try {
            ...
            switch (modelConfig.provider) {
                ...
                default:
                    throw new Error(
                        `Unsupported AI provider: ${modelConfig.provider}`,
                    );
            }
    ...
    private async reviewCodeWithOpenAISDK(
        ...
        const isAnthropicFormat =
            modelConfig.apiEndpoint?.includes("anthropic");
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (lineMatch) {
                ...
                if (currentComment.lineNumber && currentContent.length > 0) {
                    const content = this.cleanCommentContent(
                        currentContent.join("\n").trim(),
                    );
    ...
    parseReviewSummary(
        ...
        for (const line of lines) {
            ...
            if (/^\s*建议问题/i.test(line))
                currentSectionSeverity = "suggestion";

            if (
                commentItems.length >= maxItems &&
                criticalItems.length >= maxCriticalItems
            )
                break;
            ...
            const explicitSeverityMatch = rawContent.match(
                /^\[?(严重|一般|建议|critical|normal|suggestion)\]?[：:\s-]*/i,
            );
            ...
            const severity =
                explicitSeverity ||
                currentSectionSeverity ||
                this.inferSeverity(rawContent);
            const content = this.cleanCommentContent(
                rawContent
                    .replace(
                        /^\[?(严重|一般|建议|critical|normal|suggestion)\]?[：:\s-]*/i,
        ...
        if (
            ...
            counts.critical = commentItems.filter(
                (item) => item.severity === "critical",
            ).length;
            counts.normal = commentItems.filter(
                (item) => item.severity === "normal",
            ).length;
            counts.suggestion = commentItems.filter(
