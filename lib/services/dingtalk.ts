...
export function signWebhookUrl(
    webhookUrl: string,
    secret?: string | null,
): string {
...
export async function sendReviewToDingTalk(params: {
    ...
    if (!enabled || !webhookUrl) {
        console.log(
            "⏭️ [DingTalk] Notification disabled or webhook not configured, skip",
        );
        return;
    }
    ...
    const lines: string[] = [];
    const eventLabel =
        reviewLog.mergeRequestIid && reviewLog.mergeRequestIid !== 0
    ...
    lines.push(
        `- 作者：${reviewLog.author}${reviewLog.authorUsername ? `（${reviewLog.authorUsername}）` : ""}`,
    );
    lines.push(
        `- 分支：${reviewLog.sourceBranch}${reviewLog.targetBranch ? ` → ${reviewLog.targetBranch}` : ""}`,
    );
    lines.push("");
    ...
    if (params.messageOverride) {
        ...
        const summary = reviewLog.aiSummary
            ? compactText(reviewLog.aiSummary, 240)
            : "";
        lines.push(`- 结论：${conclusion}`);
        lines.push(
            `- 问题统计：🔴 ${critical} / ⚠️ ${normal} / 💡 ${suggestion}`,
        );
        lines.push(
            `- 审查文件：${reviewLog.reviewedFiles}/${reviewLog.totalFiles}`,
        );
        if (summary) {
            lines.push("");
    ...
    try {
        ...
        if (!result.ok) {
            console.warn(
                `⚠️ [DingTalk] Failed to send message: ${result.status} ${result.text ?? ""}`,
            );
        } else {
