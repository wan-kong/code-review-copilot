...
function buildChartData(data: ContributionResponse, rangeMonths: number) {
    ...
    const filtered = data.dates
        ...
        .filter(
            (item) =>
                new Date(`${item.date}T00:00:00`).getTime() >= cutoff.getTime(),
        );
...
export function ContributionsChart() {
    ...
    const totals = useMemo(() => {
        ...
        const total = chartData.reduce(
            (sum, item) => sum + (Number(item.__total) || 0),
            0,
        );
        const max = chartData.reduce(
            (m, item) => Math.max(m, Number(item.__total) || 0),
            0,
        );
        ...
        <Card className="border-border/40">
            <CardContent className="p-6">
                ...
                {data && data.authors.length > 0 ? (
                    <div className="mt-4 max-h-[160px] overflow-y-auto rounded-lg border border-border/40 p-3">
