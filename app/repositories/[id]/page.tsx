...
export default function RepositoryDetailPage() {
    ...
    const repositoryId = Array.isArray(params.id)
        ? params.id[0]
        : (params.id as string | undefined); // 兼容数组与空值的路由参数
    ...
    const loadRepository = async () => {
        ...
        try {
            ...
            if (hasCustomModel) {
                ...
                setCustomModel({
                    provider: data.customProvider as
                        | "openai"
                        | "claude"
                        | "custom",
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: only re-run when repositoryId changes
    useEffect(() => {
        if (!repositoryId) {
            // 当仓库 ID 不存在
            return; // 直接返回避免请求
    ...
    const saveConfig = async () => {
        ...
        try {
            ...
            if (
                ...
                (!customModel.modelId || !customModel.apiKey)
            ) {
                ...
                body: JSON.stringify({
                    ...
                    ...(selectedModelId === "__custom__"
                        ? {
                              ...
                              customApiEndpoint:
                                  customModel.apiEndpoint || null,
                              customMaxTokens: customModel.maxTokens
                                  ? parseInt(customModel.maxTokens, 10)
                                  : null,
                              customTemperature: customModel.temperature
                                  ? parseFloat(customModel.temperature)
                                  : null,
                          }
                        : {
                              // 清除自定义模型配置
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                setConfig({
                                    ...config,
                                    watchBranches: e.target.value,
                                })
                            }
                        />
                        <p className="text-muted-foreground text-xs">
                            支持通配符 *
                            匹配，多个分支用逗号分隔。留空则监听所有分支。
                        </p>
                        ...
                        {config.customPrompt && (
                            <div className="flex items-center gap-4 pt-2">
                                ...
                                <Select
                                    ...
                                    onValueChange={(
                                        value: "extend" | "replace",
                                    ) =>
                                        setConfig({
                                            ...config,
                                            customPromptMode: value,
                                        })
                                    }
                                >
                    ...
                    <CardContent className="space-y-3 text-sm">
                        ...
                        {repository.customPrompt && (
                            <div>
                                ...
                                <p className="mt-1 max-h-32 overflow-y-auto rounded bg-muted p-2 text-xs">
                                    {repository.customPrompt}
                                </p>
                            </div>
                        )}
