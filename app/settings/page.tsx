...
export default function SettingsPage() {
    const [gitlabAccount, setGitLabAccount] = useState<GitLabAccount | null>(
        null,
    );
    const [aiModels, setAiModels] = useState<AIModel[]>([]);
    const [dingtalkSetting, setDingtalkSetting] =
        useState<DingTalkSetting | null>(null);
    ...
    const updateGitLabAccount = async () => {
        ...
        toast.success(
            `GitLab 账号 ${gitlabAccount.isActive ? "已禁用" : "已启用"}`,
        );
    };
    ...
    const saveAIModel = async () => {
        ...
        try {
            ...
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    maxTokens: modelForm.maxTokens
                        ? parseInt(modelForm.maxTokens, 10)
                        : null,
                    temperature: modelForm.temperature
                        ? parseFloat(modelForm.temperature)
                        : null,
                }),
            });
            ...
            if (editingModel) {
                setAiModels(
                    aiModels.map((m) =>
                        m.id === editingModel.id ? savedModel : m,
                    ),
                );
    ...
    const toggleModelActive = async (model: AIModel) => {
        try {
            ...
            toast.success(
                `模型 ${getModelDisplayName(model)} ${model.isActive ? "已禁用" : "已启用"}`,
            );
        } catch (error) {
            toast.error(error instanceof Error ? error.message : "未知错误");
        }
    ...
    const saveDingTalkSetting = async () => {
        ...
        try {
            const response = await fetch(
                "/api/settings/notifications/dingtalk",
                {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
    ...
    const testDingTalk = async () => {
        ...
        try {
            const response = await fetch(
                "/api/settings/notifications/dingtalk/test",
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
    ...
    return (
        <div className="p-8">
            {/* 页面标题 */}
            <div className="mb-8">
                <h1 className="font-bold text-3xl tracking-tight">配置</h1>
                                    ...
                                    <div className="rounded-lg bg-muted p-4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <code className="rounded bg-muted px-2 py-1 text-xs">
                                                {gitlabAccount.accessToken.substring(
                                                    0,
                                                    8,
                                                )}
                                                ••••
                                                    ...
                                                    {getModelSuggestions().map(
                                                        (suggestion) => (
                                                            <button
                                                                ...
                                                                type="button"
                                                                className="ml-2 text-sidebar-primary text-xs hover:underline"
                                                                onClick={() =>
                                                                    setModelForm(
                                                                        {
                                                                            ...modelForm,
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="flex gap-2">
                                <Button
                                    ...
                                    disabled={dingtalkTesting || dingtalkSaving}
                                >
                                    {dingtalkTesting ? (
                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                    ) : null}
