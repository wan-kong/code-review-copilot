...
export async function POST(request: NextRequest) {
    try {
        ...
        const gitLabAccount = gitLabAccountId
            ? await prisma.gitLabAccount.findUnique({
                  where: { id: gitLabAccountId },
              })
            : await prisma.gitLabAccount.findFirst({
                  where: { isActive: true },
              });
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (defaultAIModelId !== undefined)
            updateData.defaultAIModelId = defaultAIModelId;
        if (customPrompt !== undefined) updateData.customPrompt = customPrompt;
        if (customPromptMode !== undefined)
            updateData.customPromptMode = customPromptMode;
        if (watchBranches !== undefined)
            updateData.watchBranches = watchBranches;
        if (customProvider !== undefined)
            updateData.customProvider = customProvider;
        if (customModelId !== undefined)
            updateData.customModelId = customModelId;
        if (customApiKey !== undefined) updateData.customApiKey = customApiKey;
        if (customApiEndpoint !== undefined)
            updateData.customApiEndpoint = customApiEndpoint;
        if (customMaxTokens !== undefined)
            updateData.customMaxTokens = customMaxTokens;
        if (customTemperature !== undefined)
            updateData.customTemperature = customTemperature;
