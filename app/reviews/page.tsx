...
export default function ReviewsPage() {
    ...
    const [retryingReviewId, setRetryingReviewId] = useState<string | null>(
        null,
    );
    ...
    const retryReview = async (reviewId: string, event: React.MouseEvent) => {
        ...
        try {
            ...
            alert(
                err instanceof Error ? err.message : "重新审查失败，请稍后重试",
            );
        } finally {
    ...
    const getGitlabFileLink = (
        ...
        const base = review.gitlabUrl?.replace(/\/+$/, "");
        if (!base || !review.repositoryPath || !filePath || !lineNumber)
            return null;
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: fetchReviews changes on every render, only run on mount
    useEffect(() => {
        fetchReviews();
    }, []);
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case "pending":
                return (
                    <Badge className="border-border/40 bg-sidebar text-sidebar-foreground">
                        进行中
                    </Badge>
                );
            case "failed":
    ...
    const selectedGitlabLink = selectedReview
        ? getGitlabLink(selectedReview)
        : null;
    ...
    return (
        <div className="p-8">
                ...
                <div className="p-6">
                    {loading ? (
                        <div className="py-12 text-center text-muted-foreground">
                            <RefreshCw className="mx-auto mb-4 h-8 w-8 animate-spin opacity-50" />
                            <p>加载中...</p>
                        </div>
                    ) : error ? (
                            ...
                            <TableBody>
                                {reviews.map((review: Review) => (
                                    ...
                                    >
                                        ...
                                        <TableCell className="px-4 py-3">
                                            <p className="max-w-50 truncate text-foreground text-sm">
                                                {review.title}
                                            </p>
                                        ...
                                        <TableCell className="px-4 py-3">
                                            {(review.status === "failed" ||
                                                ...
                                                >
                                                    <RefreshCw
                                                        className={`mr-1 h-3 w-3 ${retryingReviewId === review.id ? "animate-spin" : ""}`}
                                                    />
                                                    {retryingReviewId ===
                                                    review.id
                                                        ? "重审中"
            ...
            >
                <DialogContent
                    className="max-h-[90vh] w-full min-w-0 max-w-[96vw] overflow-hidden border-border/60 p-0 shadow-2xl"
                    showCloseButton={true}
                >
                    {selectedReview && (
                        <div className="flex h-[88vh] min-w-0 flex-col">
                            <DialogHeader className="border-border/40 border-b bg-sidebar/30 px-6 py-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {(selectedReview.status ===
                                                    ...
                                                    >
                                                        <RefreshCw
                                                            className={`mr-1 h-3 w-3 ${retryingReviewId === selectedReview.id ? "animate-spin" : ""}`}
                                                        />
                                                        {retryingReviewId ===
                                                        selectedReview.id
                                                            ? "重审中..."
                                            ...
                                            <div className="mt-2 space-y-3 text-muted-foreground text-sm">
                                                ...
                                                <div className="flex flex-wrap items-center gap-3 text-xs">
                                                    ...
                                                    <div className="rounded-md border border-border/60 bg-background/60 px-2.5 py-1">
                                                        变更文件：
                                                        {
                                                            selectedReview.reviewedFiles
                                                        }
                                                        /
                                                        {
                                                            selectedReview.totalFiles
                                                        }
                                                    </div>
                                                    {selectedReview.status ===
                                                        "completed" &&
                                                        selectedReview.criticalIssues ===
                                                            0 &&
                                                        selectedReview.normalIssues ===
                                                            0 &&
                            ...
                            <div className="min-h-0 min-w-0 flex-1 overflow-hidden px-2 py-4">
                                <Tabs
                                    defaultValue={
                                ...
                                >
                                    <TabsList className="no-scrollbar sticky top-0 z-10 mb-4 flex h-10 w-full flex-nowrap gap-2 overflow-x-auto whitespace-nowrap border border-border/40 bg-background/80 p-1 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                                        <TabsTrigger
                                            value="summary"
                                            className="h-full"
                                        >
                                    ...
                                    >
                                        {selectedReview.aiSummary ? (
                                            <div className="overflow-x-auto rounded-lg border border-border/40 bg-background p-4">
                                                <pre className="whitespace-pre-wrap break-words text-foreground/90 text-sm leading-relaxed">
                                                    {selectedReview.aiSummary}
                                                </pre>
                                            </div>
                                        ) : (
                                        ...
                                        selectedReview.comments.length > 0 ? (
                                            <div className="overflow-x-auto rounded-lg border border-border/40 bg-background p-4">
                                                <div className="space-y-3">
                                                    {selectedReview.comments.map(
                                                        (comment) => (
                                                            <div
                                                                key={comment.id}
                                                                className={`rounded-md border-l-4 p-3 ${getSeverityStyle(comment.severity)}`}
                                                            >
                                            ...
                                            <div className="space-y-2 py-8 text-center text-muted-foreground text-sm">
                                                <div>暂无可定位的审查意见</div>
                                                {selectedReview.criticalIssues +
                                                    selectedReview.normalIssues +
                                                    selectedReview.suggestions >
                                                    ...
                                                    {Object.entries(
                                                        ...
                                                        ([
                                                            ...
                                                            <div key={filePath}>
                                                                ...
                                                                <pre className="min-w-max whitespace-pre rounded-md bg-sidebar/50 p-3 text-muted-foreground text-xs">
                                                                    {response}
                                                                </pre>
                                                            </div>
                                                        ),
                                    ...
                                    <TabsContent
                                        ...
                                        className="min-h-0 min-w-0 flex-1 overflow-y-auto"
                                    >
                                        {selectedReview.aiModelProvider ||
                                        selectedReview.aiModelId ? (
                                            <div className="overflow-x-auto rounded-lg border border-border/40 bg-background p-4">
